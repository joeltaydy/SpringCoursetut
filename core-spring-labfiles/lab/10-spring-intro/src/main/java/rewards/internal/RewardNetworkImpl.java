...
public class RewardNetworkImpl implements RewardNetwork {
	...
	public RewardConfirmation rewardAccountFor(Dining dining) {
		...
		Account foundCreditCard = accountRepository.findByCreditCard(creditCardNumber);
		Restaurant foundRestaurant= restaurantRepository.findByMerchantNumber(merchantNumber);
		MonetaryAmount ma = foundRestaurant.calculateBenefitFor(foundCreditCard, dining);
		AccountContribution accountContribution = foundCreditCard.makeContribution(ma);
		accountRepository.updateBeneficiaries(foundCreditCard);
